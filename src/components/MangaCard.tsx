...
import { Eye, Star, Calendar, User, TrendingUp, Clock, BookOpen } from 'lucide-react';
...
export const MangaCard: React.FC<MangaCardProps> = ({ 
  ...
  const isNew = manga.lastUpdated && 
    new Date(manga.lastUpdated) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        ...
        <div className="aspect-[3/4] overflow-hidden bg-gray-200 relative">
          ...
          {manga.status && (
            <div className="absolute top-2 left-2">
              <span className={`px-2 py-1 text-xs font-medium rounded-full border ${statusColors[manga.status as keyof typeof statusColors]}`}>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between text-xs text-gray-500">
                  ...
                  {manga.lastUpdated && (
                    <div className="flex items-center space-x-1">
                      ...
                      <span>{new Date(manga.lastUpdated).toLocaleDateString('vi-VN')}</span>
                    </div>
                  )}
                </div>
                ...
                {manga.genres && manga.genres.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-3">
                    {manga.genres.slice(0, variant === 'compact' ? 2 : 3).map((genre) => (
                      <span
                        key={genre}
                ...
                {manga.latestChapter && (
                  <div className="mt-2 pt-2 border-t border-gray-100">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-medium text-primary-600">{manga.latestChapter}</span>
                    </div>
                  </div>
                )}
              </div>
